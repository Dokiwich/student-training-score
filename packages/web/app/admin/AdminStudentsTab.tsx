...
export function AdminStudentsTab({ classId, classNameStr, onBack }: AdminStudentsTabProps) {
  ...
  const columns = [
    ...
    { header: 'Trạng thái', width: 140, render: (s: any) => {
        const st = s.status;
        let bg = 'var(--border)', color = 'var(--muted-foreground)', label = st;
        ...
        if (st === 'FINALIZED') { bg = '#ecfdf5'; color = 'var(--success)'; label = 'Đã chốt'; }
    ...
    { header: 'Xếp loại', width: 120, align: 'center' as const, render: (s: any) => {
        ...
        const col: any = { EXCELLENT: 'var(--success)', VERY_GOOD: '#3b82f6', GOOD: '#f59e0b', AVERAGE: 'var(--muted-foreground)', WEAK: 'var(--danger)', POOR: 'var(--danger)' };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {selectedStudentForEdit && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 9999, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div style={{ backgroundColor: 'var(--surface)', borderRadius: 16, width: '90vw', maxWidth: 1000, height: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', animation: 'slideIn 0.3s ease-out' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface-muted)', borderRadius: '16px 16px 0 0' }}>
              <div>
              ...
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                {selectedStudentForEdit.sheetId && (
                  <button 
                    ...
                    style={{ background: 'var(--danger)', color: 'white', padding: '6px 12px', borderRadius: '6px', fontSize: '13px', fontWeight: 600, border: 'none', cursor: 'pointer' }}
                  >
                    Hạ Bậc Xếp Loại
                  </button>
                )}
                <button 
                  ...
                  style={{ width: 36, height: 36, borderRadius: '50%', border: 'none', background: 'var(--secondary)', color: 'var(--muted-foreground)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}
          ...
          {showDemoteModal && (
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 10000, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
              <div style={{ backgroundColor: 'var(--surface)', borderRadius: 12, width: 400, padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
                <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: 'var(--danger)' }}>Hạ bậc xếp loại</h3>
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 8, justifyContent: 'flex-end' }}>
                  ...
                  <button onClick={handleDemote} disabled={demoting} className="btn-primary" style={{ background: 'var(--danger)' }}>
