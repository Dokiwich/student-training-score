...
export function ScoringForm({
  ...
  // Fixed: properly build children map once
  ...
  const getRoot = useCallback((itemId: number): Criterion | null => {
    const item = criteria.find(c => c.id === itemId);
    if (!item) return null;
  ...
  const calculateScoreFromMap = useCallback(
    (itemId: number, scoreMap: Record<number, number>, visited = new Set<number>()): number => {
      ...
      if (children.length > 0) {
        const sum = children.reduce((acc, child) => acc + calculateScoreFromMap(child.id, scoreMap, visited), 0);
  ...
  const calculateAutoScore = useCallback(
    (itemId: number, visited = new Set<number>()): number => {
      ...
      const children = criteria.filter((c) => c.parent_id === itemId);
      if (children.length > 0) {
        const sum = children.reduce((acc, child) => acc + calculateAutoScore(child.id, visited), 0);
        return item.max_points > 0 ? Math.min(sum, item.max_points) : sum;
      }
              ...
              {TAB_GROUPS.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`shrink-0 text-left p-3 border transition-colors min-w-[240px] lg:min-w-0 lg:w-full ${
                      isActive ? 'border-black bg-gray-50' : 'border-transparent hover:border-gray-200 hover:bg-gray-50'
                    }`}
                  >
                              ...
                              return (
                                <td key={role} className="p-3 border-l border-gray-100 bg-gray-50">
                                  <input
                                    ...
                                    className={`w-full border px-2 py-1.5 text-center text-sm outline-none transition-colors ${
                                      !canEdit
                            ...
                            <td className="p-3 text-center">
                              {!isFixed && (
                                <button
                                  ...
                                  className={`px-3 py-1.5 text-xs font-bold transition-colors w-full ${
                                    isSaved ? 'bg-gray-100 text-gray-500 cursor-default' : 'bg-black text-white hover:bg-gray-800'
                                  }`}
                                >
